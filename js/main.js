...
$(document).on("click", ".ip", () => {
    ...
    copy.textContent = "Aness6040Admin.aternos.me:58029";
    copy.select();
    document.execCommand("copy");
    $(".ip").html("<span class='extrapad'>IP copied! - L'IP est copié! </span>");
    setTimeout(() => {
        $(".ip").html(t);
...
const updatePlayercount = (ip, port) => {
    $.get(`https://api.bybilly.uk/api/players/${ip}/${port}`, (result) => {
        if (result.hasOwnProperty('online')) {
            ...
            $(".playercount").html("<p>Server isn't online! - Le serveur n'est pas en ligne!</p><a class='btn-alt' data-bs-toggle='modal' data-bs-target='#onlinestaticBackdrop'>Click to view the reasons - Cliquez pour voir les raisons.</a>");
        }
    });
};